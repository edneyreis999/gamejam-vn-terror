...
Use ordinary native Call Common Event branches for execution. Under the later ADR-G001 amendment below, the eight Maps037–044 contain their actual interactions inline after tasks19/20. Shared native interaction events remain allowed; authors follow actual gameplay entry paths or edit the database directly, with no editorial-only map wrappers under ADR-G002. The native command117 selector is the content association; do not mirror it in ConfigureHero/Encounter, restore ConfigureScene or introduce Executar conteúdo configurado. Bridge may supply the hero identity or campaign facts needed by a branch, but does not resolve or execute its content. Standalone role units replace the profile/speech/selection/etc. ranges currently mixed in CEs 5–12. Never execute the old whole Common Event and hope its unrelated sections do not run.
...
**Architecture and organization accepted on 2026-09-14 under [ADR-G001](../../../docs/adrs/adr-g001-mapas-de-interacao-dos-herois.md) and [ADR-G002](../../../docs/adrs/adr-g002-remocao-de-atalhos-editoriais.md).** The [original experiment](adrs/adr-005.md) is historical. Other human and creative judgments retain their separate status in verification.md. Map037 owns H1 interaction commands inline; CE003 transfers and ends; H1 units use explicit ObservationBegin numeric unit82–85, default0 retaining Common Event inference. Rules, checkpoint timing and provider order remain. The42 map shortcuts are removed with null slots and surviving native call paths retained. CE004/351 remain directly editable in the database. Map/event structural save compatibility is not promised. See [ADR-005](adrs/adr-005.md), spec EX-001–006 and verification EXV-001–004.
...
Tasks19–29 implement spec MA-001–011. Each migrated map owns its actual native content and local flow; CE040 retains cross-scene routing and unconverted/shared paths, with no duplicate execution of a migrated body. Task22 records/proves the native phase/map/interpreter handoff with H1's epilogue before the remaining campaign maps; task26 extends the proof to A1's encounter phases. Keep campaign truth and semantic passage completion in CampaignRules/Bridge, observational numeric read history in its existing Presentation store, and save transport in MZ/SaveCore. Explicitly preserve old observation IDs86–113 when their CE bodies become null.

Retire displaced bodies per slice and transitive orphans at task29. Audit native calls, choice tags, configuration/provider selectors, automatic/parallel triggers and project reserve/call code; update tests and current consumers. Preserve array IDs, functional helpers and memorial death-inscription consumers. Include CE321/irati.03 in the Council migration before retiring CE041. No runtime content manifest/registry, new domain phase, copied CE040 state machine, transfer autosave, legacy save conversion, new dependency or vendor edit. Materialized data transformations and removal ledgers are implementation evidence, not authoring prerequisites. Canonical suite ownership and integration/QA freshness are in MAV-001–014.
