...
// One-time task19 transformation. Later native-editor changes are authoritative.
const events = read('CommonEvents.json');
const infos = read('MapInfos.json');
const template = read('Map037.json');
const before = structuredClone(events);
const heroes = [
  { id: 'H2', name: 'Elowen', slug: 'elowen', ce: 6, unit: 86, map: 38, x: 250, y: 1320, scale: 38, listenerY: 1254 },
  { id: 'H3', name: 'Griznik', slug: 'griznik', ce: 7, unit: 90, map: 39, x: 320, y: 1100, scale: 50, listenerY: 1058 },
  { id: 'H4', name: 'Seraphina', slug: 'seraphina', ce: 8, unit: 94, map: 40, x: 355, y: 1260, scale: 38, listenerY: 1198 }
];
assert.equal(infos.length, 38, 'Allocate only after reconciling the live map inventory');
...
assert.ok(consumers.every(({ owner, target }) => owner === 'CE3' ? [6, 7, 8].includes(target) : heroes.some(hero => owner === `CE${hero.ce}` && target >= hero.unit && target < hero.unit + 4)));
