(function (global) {
  ...
  function fixture(width, initialState) {
    ...
    var controller = global.ExpeditionApp.createController(root, initialState);
    return {
      root: root,
  ...
  function ensureHeroes(controller, heroIds) {
    var selected = controller.getState().draftPartyIds.slice();
    selected.filter(function (heroId) { return heroIds.indexOf(heroId) < 0; }).forEach(function (heroId) {
      Test.truthy(controller.dispatch({ type: 'TOGGLE_HERO', heroId: heroId }).ok);
    });
    heroIds.filter(function (heroId) { return controller.getState().draftPartyIds.indexOf(heroId) < 0; }).forEach(function (heroId) {
      Test.truthy(controller.dispatch({ type: 'TOGGLE_HERO', heroId: heroId }).ok);
    });
  }

  function selectDestination(controller, dungeonId) {
    var state = controller.getState();
    Test.equal(state.phase, 'formation');
    if (state.selectedDungeonId !== dungeonId) {
      Test.truthy(controller.dispatch({ type: 'SELECT_DESTINATION', dungeonId: dungeonId }).ok);
    }
  }
  ...
  function ensureParty(controller) {
    ...
    selectDestination(controller, availableDestination(state, 'physical'));
    if (living.length >= 3) {
  ...
  function driveDeaths(controller, targetCount) {
    ...
    while (controller.getState().deadHeroIds.length < targetCount && steps < MAX_DEATH_STEPS) {
      ...
      if (state.phase === 'ready') {
        ...
        Test.truthy(controller.dispatch({ type: 'ACK_DUNGEON_COMPLETE' }).ok);
      } else {
  ...
  function returnToFormation(controller) {
    ...
    while (controller.getState().phase !== 'formation' && guard < 80) {
      ...
      if (state.phase === 'dungeon_intro') {
        ...
        controller.dispatch({ type: 'ACK_DUNGEON_COMPLETE' });
      } else {
  ...
  function reachThreshold(controller, heroIds) {
    ...
    selectDestination(controller, 'physical');
  ...
  function finishWithoutDeaths(controller) {
    ...
    while (controller.getState().phase !== 'victory' && guard < 100) {
      ...
      if (state.phase === 'formation') {
        selectDestination(controller, availableDestination(state, 'physical'));
        ensureHeroes(controller, ['H1', 'H2', 'H3']);
        Test.truthy(controller.dispatch({ type: 'DEPART' }).ok);
      } else if (state.phase === 'dungeon_intro') {
        Test.truthy(controller.dispatch({ type: 'ENTER_DUNGEON' }).ok);
      } else if (state.phase === 'encounter_choice') {
  ...
  function prepareSoloPartyWithReserve(controller) {
    ...
    selectDestination(controller, availableDestination(controller.getState(), 'physical'));
  ...
  function reachDungeonComplete(controller, dungeonId, seed) {
    ...
    while (!(controller.getState().phase === 'dungeon_complete' && controller.getState().dungeonId === dungeonId) && guard < 100) {
      ...
      var state = controller.getState();
      if (state.phase === 'formation') {
        selectDestination(controller, availableDestination(state, dungeonId));
    ...
    for (var seed = 1; seed <= 2000; seed += 1) {
      ...
      while (valid && state.phase !== 'victory' && guard < 120) {
        ...
        if (state.phase === 'formation') {
          ...
          apply({ type: 'SELECT_DESTINATION', dungeonId: nextDungeon });
          parties[nextDungeon].forEach(function (heroId) { apply({ type: 'TOGGLE_HERO', heroId: heroId }); });
          apply({ type: 'DEPART' });
        } else if (state.phase === 'dungeon_intro') {
          ...
          apply({ type: 'ACK_DUNGEON_COMPLETE' });
        } else {
          valid = false;
        }
      }
  ...
  function reachCatalogEncounter(controller, seed, encounterId) {
    ...
    while (guard < 100) {
      ...
      var state = controller.getState();
      if (state.phase === 'formation') {
        selectDestination(controller, availableDestination(state, encounterId.charAt(0) === 'A' ? 'physical' : 'supernatural'));
        selectHeroes(controller, ['H1', 'H2', 'H3']);
        Test.truthy(controller.dispatch({ type: 'DEPART' }).ok);
      } else if (state.phase === 'dungeon_intro') {
        Test.truthy(controller.dispatch({ type: 'ENTER_DUNGEON' }).ok);
  ...
  Test.test('IT-002 — imagem local opcional ausente conserva texto e controles', function () {
    ...
    Test.includes(scene.textContent, 'Caminho do Ferro e das Raízes');
  ...
  Test.test('IT-023 — exatamente três sobreviventes exigem os três na partida', function () {
    ...
    selectDestination(current.controller, availableDestination(current.controller.getState(), 'physical'));
  ...
  Test.test('IT-038 — formação inicial mostra elenco integral sem alegar grupo atual', function () {
    ...
    Test.includes(current.root.querySelector('.formation-panel').textContent, 'Escolha os heróis');
    current.cleanup();
  });
  ...
  Test.test('IT-047 — revisitas após recuos conservam o primeiro encontro', function () {
    ...
    for (var attempt = 0; attempt < 2; attempt += 1) {
      ...
      selectDestination(current.controller, 'physical');
      selectHeroes(current.controller, ['H1', 'H2', 'H3']);
      Test.truthy(current.controller.dispatch({ type: 'DEPART' }).ok);
      Test.includes(current.root.textContent, 'O primeiro encontro já foi revelado nesta campanha.');
      Test.falsy(current.root.textContent.indexOf('O primeiro encontro ainda não foi revelado.') >= 0);
      Test.truthy(current.controller.dispatch({ type: 'ENTER_DUNGEON' }).ok);
  ...
  Test.test('IT-091 — entrada final prematura é interrompida como estado inválido', function () {
    ...
    Test.includes(result.state.invariantViolations.map(function (violation) { return violation.code; }), 'final_destination_locked');
    current.cleanup();
  });
  ...
  Test.test('IT-092 — inventário inicial corrompido para sem candidatos para em diagnóstico', function () {
    ...
    var result = Engine.dispatch(invalid, { type: 'ACK_DUNGEON_COMPLETE' });
    Test.truthy(result.ok);
    Test.equal(result.state.phase, 'invalid');
  ...
  Test.test('IT-094 — conclusão usa nomes diegéticos e reconhecimento explícito', function () {
    var current = fixture(320);
    reachDungeonComplete(current.controller, 'physical', 70);
    Test.equal(current.root.querySelectorAll('[data-action="ack-dungeon-complete"]').length, 1);
    Test.includes(current.root.textContent, 'Uma parte do mapa foi recuperada.');
    Test.includes(current.root.textContent, 'Caminho do Ferro e das Raízes');
    Test.falsy(current.root.textContent.indexOf('Primeira metade') >= 0);
  ...
  Test.test('IT-095 — reconhecimento único retorna à formação e seleciona o próximo caminho', function () {
    var current = fixture();
    reachDungeonComplete(current.controller, 'physical', 71);
    Test.truthy(current.controller.dispatch({ type: 'ACK_DUNGEON_COMPLETE' }).ok);
    Test.falsy(current.controller.dispatch({ type: 'ACK_DUNGEON_COMPLETE' }).ok);
    Test.equal(current.controller.getState().dungeonId, null);
    Test.equal(current.controller.getState().selectedDungeonId, 'supernatural');
    Test.equal(current.controller.getState().actionHistory.filter(function (entry) { return entry.type === 'dungeon_completion_acknowledged'; }).length, 1);
    current.cleanup();
  });
  ...
  Test.test('IT-098 — final não preatribui posições futuras', function () {
    ...
    Test.truthy(current.controller.dispatch({ type: 'ACK_DUNGEON_COMPLETE' }).ok);
    reachDungeonComplete(current.controller, 'physical', 74);
    Test.truthy(current.controller.dispatch({ type: 'ACK_DUNGEON_COMPLETE' }).ok);
  ...
  Test.test('IT-099 — um ou dois sobreviventes são autoformados entre masmorras', function () {
    ...
    var result = Engine.dispatch(reduced, { type: 'ACK_DUNGEON_COMPLETE' });
    Test.truthy(result.ok);
  ...
  Test.test('UT-099 — snapshot QA v2 agrega rotas, formação e rejeição nula', function () {
    var current = fixture();
    beginToFormation(current.controller, 20260831);
    selectDestination(current.controller, 'supernatural');
    selectHeroes(current.controller, ['H1', 'H2', 'H3']);
    var snapshot = global.expeditionQA.snapshot();
    Test.equal(snapshot.version, 2);
    Test.equal(snapshot.selectedDestination, 'supernatural');
    Test.deepEqual(snapshot.draftParty, ['H1', 'H2', 'H3']);
    Test.deepEqual(Object.keys(snapshot.destinations), ['physical', 'supernatural', 'final']);
    Test.equal(snapshot.lastRejectedAction, null);
    current.cleanup();
  });

  Test.test('UT-100 — snapshot QA devolve dados destacados do controlador', function () {
    var current = fixture();
    beginToFormation(current.controller, 100);
    var first = global.expeditionQA.snapshot();
    try { first.destinations.physical.landmarks.traversed = 99; } catch (error) { /* congelamento esperado */ }
    try { first.assignments.physical[0] = 'B8'; } catch (error) { /* congelamento esperado */ }
    var later = global.expeditionQA.snapshot();
    Test.equal(later.destinations.physical.landmarks.traversed, 0);
    Test.equal(later.assignments.physical[0], null);
    current.cleanup();
  });

  Test.test('UT-101 — rejeição de destino gera um diagnóstico estruturado e um aviso', function () {
    var current = fixture();
    beginToFormation(current.controller, 101);
    var warnings = [];
    ...
    var rejection = global.expeditionQA.snapshot().lastRejectedAction;
    Test.deepEqual(rejection, { action: 'SELECT_DESTINATION', code: 'destination_unavailable', message: 'Este caminho não está disponível para expedição.', context: { dungeon: 'final', status: 'locked' } });
    Test.equal(warnings.filter(function (entry) { return entry[0] === 'campaign_action_rejected'; }).length, 1);
    current.cleanup();
  });
  ...
  Test.test('UT-105 — seed pública preserva formas de sucesso e erro', function () {
    var current = fixture();
    Test.deepEqual(global.expeditionQA.setSeed(20260831), { ok: true, seed: 20260831 });
    current.root.querySelector('[data-action="begin"]').click();
    Test.equal(global.expeditionQA.snapshot().seed, 20260831);
    Test.deepEqual(global.expeditionQA.setSeed(1), { ok: false, error: { code: 'campaign_already_started', message: 'Defina a semente antes de iniciar a campanha.' } });
    current.cleanup();
  });
  ...
  function routeCase(id, title, callback, options) {
    Test.test('IT-' + id + ' — ' + title, async function () {
      ...
      try {
        beginToFormation(current.controller, options && options.seed !== undefined ? options.seed : Number(id));
  ...
  function clickRoute(current, dungeonId) {
    var radio = current.root.querySelector('[data-destination-control][value="' + dungeonId + '"]');
    Test.truthy(radio);
    radio.click();
  }
  ...
  function completeInitialOrder(current, order) {
    order.forEach(function (dungeonId) {
      reachDungeonComplete(current.controller, dungeonId, 198);
      current.root.querySelector('[data-action="ack-dungeon-complete"]').click();
    });
    Test.equal(current.controller.getState().phase, 'formation');
    Test.equal(current.controller.getState().selectedDungeonId, 'final');
  }

  function enterRouteAndRevealFirst(current, dungeonId, heroIds) {
    selectDestination(current.controller, dungeonId);
    ensureHeroes(current.controller, heroIds || ['H1', 'H2', 'H3']);
    Test.truthy(current.controller.dispatch({ type: 'DEPART' }).ok);
    Test.truthy(current.controller.dispatch({ type: 'ENTER_DUNGEON' }).ok);
    return current.controller.getState().assignments[dungeonId][0];
  }

  function retreatToFormation(current) {
    Test.truthy(current.controller.dispatch({ type: 'REQUEST_RETREAT' }).ok);
    Test.truthy(current.controller.dispatch({ type: 'CONFIRM_RETREAT' }).ok);
    Test.equal(current.controller.getState().phase, 'formation');
  ...
  routeCase('151', 'partida sem caminho foca alerta e preserva heróis', function (current) {
    selectHeroes(current.controller, ['H1', 'H2', 'H3']);
    current.root.querySelector('[data-action="depart"]').click();
    Test.equal(document.activeElement.id, 'campaign-error');
  ...
  routeCase('152', 'zero sobreviventes deriva derrota sem controles de preparação', function (current) {
    var injectedFormation = JSON.parse(JSON.stringify(current.controller.getState()));
    injectedFormation.deadHeroIds = Data.heroOrder.slice();
    injectedFormation.draftPartyIds = [];
    driveDeaths(current.controller, 8);
    Test.equal(current.controller.getState().phase, 'defeat');
    Test.equal(current.root.querySelectorAll('.formation-panel').length, 0);
    Test.equal(current.root.querySelectorAll('[data-destination-control], [data-action="toggle-hero"], [data-action="depart"]').length, 0);
    var mounted = fixture(1000, injectedFormation);
    try {
      Test.equal(mounted.controller.getState().phase, 'invalid');
      Test.includes(mounted.controller.getState().invariantViolations.map(function (violation) { return violation.code; }), 'invalid_formation_state');
      Test.equal(mounted.root.querySelectorAll('.formation-panel, [data-destination-control], [data-action="toggle-hero"], [data-action="depart"]').length, 0);
    } finally {
      mounted.cleanup();
    }
  });
  routeCase('153', 'rota única e elenco reduzido são selecionados sem partir', function (current) {
    ...
    Test.deepEqual(state.draftPartyIds, ['H7', 'H8']);
    Test.deepEqual(Array.prototype.map.call(current.root.querySelectorAll('.hero-card.automatic'), function (card) {
      return card.dataset.heroId;
    }), ['H7', 'H8']);
  ...
  routeCase('154', 'controle manipulado não ativa rota bloqueada', function (current) {
    ...
    Test.falsy(current.controller.dispatch({ type: 'SELECT_DESTINATION', dungeonId: 'final' }).ok);
    Test.equal(JSON.stringify(current.controller.getState()), before);
    Test.equal(current.root.querySelectorAll('[value="final"]').length, 0);
  });
  routeCase('155', 'duas mudanças deixam somente o último rádio marcado', function (current) {
    clickRoute(current, 'physical'); clickRoute(current, 'supernatural');
    Test.equal(current.root.querySelectorAll('[data-destination-control]:checked').length, 1);
    Test.equal(current.controller.getState().selectedDungeonId, 'supernatural');
  });
  routeCase('156', 'consulta do elenco preserva escolhas antes da partida', function (current) {
    clickRoute(current, 'physical'); selectHeroes(current.controller, ['H1', 'H2', 'H3']);
    current.root.querySelector('[data-action="open-roster"]').click(); current.root.querySelector('[data-action="close-roster"]').click();
  ...
  routeCase('158', 'última escolha de rota é comprometida na partida', function (current) {
    selectHeroes(current.controller, ['H1', 'H2', 'H3']);
    clickRoute(current, 'physical');
    clickRoute(current, 'supernatural');
    clickRoute(current, 'physical');
    var selections = current.controller.getState().actionHistory.filter(function (event) {
      return event.type === 'destination_selected';
    }).map(function (event) { return event.dungeon; });
    Test.deepEqual(selections, ['physical', 'supernatural', 'physical']);
    current.root.querySelector('[data-action="depart"]').click();
    Test.equal(current.controller.getState().dungeonId, 'physical');
    Test.deepEqual(current.controller.getState().partyIds, ['H1', 'H2', 'H3']);
    Test.deepEqual(current.controller.getState().actionHistory.filter(function (event) {
      return event.type === 'destination_selected';
    }).map(function (event) { return event.dungeon; }), selections);
  });
  routeCase('159', 'partida obsoleta para rota concluída é recusada', function (current) {
    var state = JSON.parse(JSON.stringify(current.controller.getState()));
    state.selectedDungeonId = 'physical'; state.routeProgress.physical = 5; state.assignments.physical = ['A1', 'A2', 'A3', 'A4', 'A5'];
    Test.includes(Engine.validateState(Object.freeze(state)).violations.map(function (v) { return v.code; }), 'destination_unavailable');
  });
  routeCase('160', 'rotas precedem oito heróis e ação no DOM', function (current) {
    Test.equal(current.root.querySelectorAll('.route-card').length, 3);
    Test.equal(current.root.querySelectorAll('.hero-card').length, 8);
    var panel = current.root.querySelector('.formation-panel');
    Test.truthy(panel.querySelector('.route-grid').compareDocumentPosition(panel.querySelector('.hero-grid')) & Node.DOCUMENT_POSITION_FOLLOWING);
  });

  routeCase('161', 'estado de destino desconhecido interrompe em vez de escolher', function (current) {
    var corrupted = JSON.parse(JSON.stringify(current.controller.getState()));
    corrupted.selectedDungeonId = 'unknown';
    var invalid = fixture(1000, corrupted);
    try {
      corrupted.selectedDungeonId = 'physical';
      Test.equal(invalid.controller.getState().phase, 'invalid');
      Test.includes(invalid.controller.getState().invariantViolations.map(function (violation) { return violation.code; }), 'invalid_destination_selection');
      Test.includes(invalid.root.textContent, Data.copy.invalidState);
      Test.equal(invalid.root.querySelectorAll('[data-destination-control]').length, 0);
      Test.equal(invalid.root.querySelector('[data-action="reload"]').textContent, 'Recarregar a página');
      var beforeValidation = JSON.stringify(invalid.controller.getState());
      Test.falsy(global.expeditionQA.validate().ok);
      Test.equal(JSON.stringify(invalid.controller.getState()), beforeValidation);
      Test.equal(invalid.controller.getState().invariantViolations[0].context.priorSnapshot.selectedDestination, 'unknown');
    } finally {
      invalid.cleanup();
    }
    var malformedInvalid = fixture(1000, { phase: 'invalid', invariantViolations: [] });
    try {
      Test.equal(malformedInvalid.controller.getState().phase, 'invalid');
      Test.includes(malformedInvalid.controller.getState().invariantViolations.map(function (violation) { return violation.code; }), 'invalid_state_shape');
      Test.includes(malformedInvalid.root.textContent, Data.copy.invalidState);
    } finally {
      malformedInvalid.cleanup();
    }
  });
  routeCase('162', 'nome estado e progresso independem de decoração', function (current) {
    Test.includes(current.root.textContent, Data.destinations.physical.name);
    Test.includes(current.root.textContent, 'Disponível');
    Test.includes(current.root.textContent, '0 de 5 marcos atravessados');
  });
  routeCase('163', 'limites de progresso renderizam cinco e seis sem exceder', function (current) {
    var state = JSON.parse(JSON.stringify(current.controller.getState()));
    state.routeProgress = { physical: 5, supernatural: 5, final: 6 };
    Test.deepEqual(Engine.deriveDestinations(Object.freeze(state)).map(function (d) { return d.landmarks.traversed + '/' + d.landmarks.total; }), ['5/5', '5/5', '6/6']);
  });
  routeCase('164', 'cartões não vazam classificações ou encontros', function (current) {
    Test.falsy(/pool [ab]|masmorra física|masmorra sobrenatural|\b[AB][1-8]\b/i.test(current.root.querySelector('.route-group').textContent));
  });
  routeCase('165', 'quinto marco entra diretamente em conclusão coerente', function (current) {
    reachDungeonComplete(current.controller, 'physical', 165);
    Test.equal(current.controller.getState().routeProgress.physical, 5);
    Test.includes(current.root.textContent, 'Uma parte do mapa foi recuperada.');
  });
  routeCase('166', 'nova montagem descarta progresso da sessão', function (current) {
    ...
    var other = fixture();
    try {
      Test.deepEqual(other.controller.getState(), Engine.createReadyState());
      Test.deepEqual(other.controller.getState().routeProgress, { physical: 0, supernatural: 0, final: 0 });
      Test.deepEqual(other.controller.getState().assignments, { physical: [null, null, null, null, null], supernatural: [null, null, null, null, null], final: [null, null, null, null, null, null] });
      Test.falsy(/retomar|recuperar sessão|continuar campanha/i.test(other.root.textContent));
    } finally {
      other.cleanup();
    }
  });
  routeCase('167', 'renderizações repetidas conservam cópia e seleção', function (current) {
    clickRoute(current, 'supernatural');
    var beforeState = Engine.snapshot(current.controller.getState());
    var beforeCopy = current.root.querySelector('.route-group').textContent;
    for (var cycle = 0; cycle < 3; cycle += 1) {
      current.root.querySelector('[data-action="open-roster"]').click();
      Test.truthy(current.root.querySelector('#roster-dialog').open);
      current.root.querySelector('[data-action="close-roster"]').click();
      Test.deepEqual(Engine.snapshot(current.controller.getState()), beforeState);
      Test.equal(current.root.querySelector('.route-group').textContent, beforeCopy);
      Test.truthy(current.root.querySelector('[value="supernatural"]:checked'));
      Test.includes(current.root.querySelector('[data-destination-id="supernatural"]').textContent, Data.destinations.supernatural.rumor);
      Test.includes(current.root.querySelector('[data-destination-id="supernatural"]').textContent, '0 de 5 marcos atravessados');
    }
  });
  routeCase('168', 'legado começa bloqueado com pré-requisito explícito', function (current) {
    var legacy = current.root.querySelector('[data-destination-id="final"]');
    Test.includes(legacy.textContent, 'Bloqueado'); Test.includes(legacy.textContent, '0 de 2 partes'); Test.falsy(legacy.querySelector('input'));
  });
  routeCase('169', 'conclusão troca rádio por cartão estático', function (current) {
    firstCompletion(current, 'physical');
    Test.falsy(current.root.querySelector('[data-destination-id="physical"] input'));
    Test.truthy(current.root.querySelector('[data-destination-control][value="supernatural"]:checked'));
  });
  routeCase('170', 'preparação estreita mantém conteúdo dentro do contêiner', function (current) {
    var heroGrid = current.root.querySelector('.hero-grid');
    var heroCards = heroGrid.querySelectorAll('.hero-card');
    Test.truthy(current.root.scrollWidth <= 320);
    Test.equal(current.root.querySelectorAll('.route-card').length, 3);
    Test.equal(heroCards.length, 8);
    Test.equal(global.getComputedStyle(heroGrid).gridTemplateColumns.split(' ').length, 1);
    Array.prototype.forEach.call(heroCards, function (card, index) {
      var bounds = card.getBoundingClientRect();
      var gridBounds = heroGrid.getBoundingClientRect();
      Test.truthy(bounds.left >= gridBounds.left && bounds.right <= gridBounds.right);
      if (index > 0) {
        Test.truthy(bounds.top >= heroCards[index - 1].getBoundingClientRect().bottom);
      }
    });
  }, { width: 320 });

  routeCase('171', 'troca fora da formação não altera consequência', function (current) {
    enterRouteAndRevealFirst(current, 'physical', ['H4', 'H1', 'H2']);
    var failure = failingApproach(current.controller.getState());
    Test.truthy(failure);
    Test.truthy(current.controller.dispatch({ type: 'CHOOSE_APPROACH', approachId: failure.id }).ok);
    ['approach_result', 'sacrifice_choice', 'sacrifice_confirmation'].forEach(function (expectedPhase) {
      Test.equal(current.controller.getState().phase, expectedPhase);
      var before = Engine.snapshot(current.controller.getState());
      var rejected = current.controller.dispatch({ type: 'SELECT_DESTINATION', dungeonId: 'supernatural' });
      Test.falsy(rejected.ok);
      Test.equal(rejected.error.code, 'invalid_transition');
      Test.deepEqual(Engine.snapshot(current.controller.getState()), before);
      if (expectedPhase === 'approach_result') {
        Test.truthy(current.controller.dispatch({ type: 'OPEN_SACRIFICE' }).ok);
      } else if (expectedPhase === 'sacrifice_choice') {
        Test.truthy(current.controller.dispatch({ type: 'SELECT_VICTIM', heroId: 'H4' }).ok);
      }
    });
  }, { seed: 1 });
  ...
  routeCase('174', 'atribuição persiste ao trocar de caminho', function (current) {
    ...
    resolveSuccess(current.controller);
    Test.truthy(current.controller.dispatch({ type: 'ENTER_DUNGEON' }).ok);
    var physicalThird = current.controller.getState().assignments.physical[2];
    Test.truthy(physicalThird);
    retreatToFormation(current);
    clickRoute(current, 'supernatural');
    Test.includes(current.controller.getState().deadHeroIds, 'H4');
    Test.equal(current.controller.getState().assignments.physical[2], physicalThird);
  }, { seed: 1 });
  ...
  routeCase('176', 'elenco após recuo não altera registros', function (current) {
    enterRouteAndRevealFirst(current, 'physical');
    retreatToFormation(current);
    var before = Engine.snapshot(current.controller.getState());
    current.root.querySelector('[data-action="open-roster"]').click();
    Test.truthy(current.root.querySelector('#roster-dialog').open);
    current.root.querySelector('[data-action="close-roster"]').click();
    Test.deepEqual(Engine.snapshot(current.controller.getState()), before);
    Test.deepEqual(current.controller.getState().deadHeroIds, before.deadHeroes);
    Test.deepEqual(current.controller.getState().assignments, before.assignments);
  });
  routeCase('177', 'alternância preserva atribuições independentes', function (current) {
    var physicalFirst = enterRouteAndRevealFirst(current, 'physical');
    retreatToFormation(current);
    var supernaturalFirst = enterRouteAndRevealFirst(current, 'supernatural');
    retreatToFormation(current);
    Test.equal(enterRouteAndRevealFirst(current, 'physical'), physicalFirst);
    Test.equal(current.controller.getState().assignments.supernatural[0], supernaturalFirst);
    retreatToFormation(current);
    Test.equal(enterRouteAndRevealFirst(current, 'supernatural'), supernaturalFirst);
    Test.equal(current.controller.getState().assignments.physical[0], physicalFirst);
    Test.truthy(physicalFirst.charAt(0) === 'A');
    Test.truthy(supernaturalFirst.charAt(0) === 'B');
  });
  ...
  routeCase('180', 'atribuições cheias sem progresso não liberam legado', function (current) {
    var state = JSON.parse(JSON.stringify(current.controller.getState())); state.assignments.physical = ['A1','A2','A3','A4','A5']; state.assignments.supernatural = ['B1','B2','B3','B4','B5']; state.routeProgress.physical = 4; state.routeProgress.supernatural = 4;
    Test.equal(Engine.deriveDestinations(Object.freeze(state))[2].status, 'locked');
  });
  ...
  routeCase('183', 'um sobrevivente permanece autoformado na rota seguinte', function (current) {
    ...
    state.draftPartyIds = ['H8'];
    var mounted = fixture(1000, state);
    try {
      Test.equal(mounted.controller.getState().phase, 'formation');
      Test.deepEqual(mounted.controller.getState().draftPartyIds, ['H8']);
      Test.equal(mounted.root.querySelectorAll('.hero-card.automatic').length, 1);
      Test.equal(mounted.root.querySelector('.hero-card.automatic').dataset.heroId, 'H8');
      Test.equal(mounted.root.querySelector('[data-destination-control]:checked').value, 'supernatural');
      Test.truthy(mounted.root.querySelector('[data-action="depart"]'));
      Test.equal(mounted.controller.getState().dungeonId, null);
    } finally {
      mounted.cleanup();
    }
  });
  routeCase('184', 'cartão concluído não possui alvo interativo', function (current) {
    firstCompletion(current, 'physical');
    var card = current.root.querySelector('[data-destination-id="physical"]');
    var before = Engine.snapshot(current.controller.getState());
    Test.equal(card.tagName, 'ARTICLE');
    Test.falsy(card.querySelector('input,button'));
    Test.falsy(card.hasAttribute('tabindex'));
    Test.falsy(card.dataset.action);
    card.click();
    card.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
    card.dispatchEvent(new KeyboardEvent('keydown', { key: ' ', bubbles: true }));
    Test.deepEqual(Engine.snapshot(current.controller.getState()), before);
    Test.falsy(document.activeElement === card);
  });
  ...
  routeCase('186', 'elenco durante recuperação não remove caminho ativo', function (current) {
    reachDungeonComplete(current.controller, 'physical', 186);
    var before = Engine.snapshot(current.controller.getState());
    Test.equal(before.dungeon, 'physical');
    Test.includes(current.root.textContent, Data.destinations.physical.name);
    current.root.querySelector('[data-action="open-roster"]').click();
    Test.truthy(current.root.querySelector('#roster-dialog').open);
    ...
    Test.deepEqual(Engine.snapshot(current.controller.getState()), before);
    Test.equal(current.controller.getState().dungeonId, 'physical');
    Test.truthy(current.root.querySelector('[data-action="ack-dungeon-complete"]'));
    Test.includes(current.root.textContent, Data.destinations.physical.name);
  });
  ...
  routeCase('191', 'legado prematuro não consome aleatoriedade', function (current) {
    [0, 1].forEach(function (expectedFragments) {
      var before = Engine.snapshot(current.controller.getState());
      var rngBefore = current.controller.getState().rngState;
      var rejected = current.controller.dispatch({ type: 'SELECT_DESTINATION', dungeonId: 'final' });
      Test.falsy(rejected.ok);
      Test.equal(rejected.error.code, 'destination_unavailable');
      Test.deepEqual(rejected.error.context, { dungeon: 'final', status: 'locked' });
      Test.equal(Engine.deriveMapFragments(current.controller.getState()), expectedFragments);
      Test.equal(current.controller.getState().rngState, rngBefore);
      Test.deepEqual(current.controller.getState().assignments.final, before.assignments.final);
      Test.falsy(current.root.querySelector('[value="final"]'));
      if (expectedFragments === 0) {
        firstCompletion(current, 'physical');
      }
    });
  });
  routeCase('192', 'progresso inconsistente expõe violação de rota', function (current) {
    var state = JSON.parse(JSON.stringify(current.controller.getState()));
    delete state.routeProgress.final;
    Test.includes(Engine.validateState(Object.freeze(state)).violations.map(function (violation) { return violation.code; }), 'invalid_route_progress');
    var mounted = fixture(1000, state);
    try {
      Test.equal(mounted.controller.getState().phase, 'invalid');
      var validation = global.expeditionQA.validate();
      Test.includes(validation.violations.map(function (violation) { return violation.code; }), 'invalid_route_progress');
      Test.truthy(validation.violations.some(function (violation) {
        return violation.context && violation.context.priorSnapshot && violation.context.priorSnapshot.rawState;
      }));
    } finally {
      mounted.cleanup();
    }
    ...
    selectHeroes(current.controller, ['H1', 'H2', 'H3']);
    Test.truthy(current.controller.dispatch({ type: 'DEPART' }).ok);
    var missingEncounter = JSON.parse(JSON.stringify(current.controller.getState()));
    missingEncounter.phase = 'encounter_choice';
    var stopped = fixture(1000, missingEncounter);
    try {
      Test.equal(stopped.controller.getState().phase, 'invalid');
      Test.includes(stopped.controller.getState().invariantViolations.map(function (violation) { return violation.code; }), 'missing_phase_payload');
      Test.includes(stopped.root.textContent, Data.copy.invalidState);
    } finally {
      stopped.cleanup();
    }
  });
  ...
  routeCase('195', 'segundo reconhecimento seleciona legado por derivação', function (current) {
    firstCompletion(current, 'physical');
    reachDungeonComplete(current.controller, 'supernatural', 195);
    var historyBefore = current.controller.getState().actionHistory.length;
    var stale = current.controller.dispatch({ type: 'SELECT_DESTINATION', dungeonId: 'final' });
    Test.falsy(stale.ok);
    Test.equal(stale.error.code, 'invalid_transition');
    Test.equal(current.controller.getState().actionHistory.length, historyBefore);
    Test.truthy(current.controller.dispatch({ type: 'ACK_DUNGEON_COMPLETE' }).ok);
    Test.equal(current.controller.getState().selectedDungeonId, 'final');
    Test.equal(current.controller.getState().actionHistory.length, historyBefore + 1);
    Test.equal(current.controller.getState().actionHistory.filter(function (event) {
      return event.type === 'destination_selected' && event.dungeon === 'final';
    }).length, 0);
  });
  routeCase('196', 'nova sessão após desbloqueio começa com zero partes', function (current) {
    ...
    Test.truthy(current.controller.dispatch({ type: 'ACK_DUNGEON_COMPLETE' }).ok);
    Test.equal(Engine.deriveMapFragments(current.controller.getState()), 2);
    current.cleanup();
    var other = fixture();
    try {
      Test.equal(Engine.deriveMapFragments(other.controller.getState()), 0);
      Test.deepEqual(other.controller.getState().routeProgress, { physical: 0, supernatural: 0, final: 0 });
      Test.equal(other.controller.getState().phase, 'ready');
      Test.equal(global.expeditionQA.snapshot().phase, 'ready');
      Test.equal(global.expeditionQA.snapshot().mapFragments.found, 0);
    } finally {
      other.cleanup();
    }
  });
  routeCase('197', 'legado desbloqueado permanece uma única seleção', function (current) {
    firstCompletion(current, 'physical');
    reachDungeonComplete(current.controller, 'supernatural', 197);
    Test.truthy(current.controller.dispatch({ type: 'ACK_DUNGEON_COMPLETE' }).ok);
    var before = Engine.snapshot(current.controller.getState());
    for (var renderCycle = 0; renderCycle < 2; renderCycle += 1) {
      current.root.querySelector('[data-action="open-roster"]').click();
      current.root.querySelector('[data-action="close-roster"]').click();
      Test.equal(current.root.querySelectorAll('[value="final"]:checked').length, 1);
      Test.deepEqual(Engine.snapshot(current.controller.getState()), before);
      Test.equal(current.controller.getState().actionHistory.filter(function (event) {
        return event.type === 'destination_selected' && event.dungeon === 'final';
      }).length, 0);
  ...
  routeCase('198', 'ordens opostas convergem para duas partes', function (current) { completeInitialOrder(current,['physical','supernatural']); var physicalFirst=current.controller.getState(); var other=fixture(); try { beginToFormation(other.controller,198); completeInitialOrder(other,['supernatural','physical']); var supernaturalFirst=other.controller.getState(); Test.equal(Engine.deriveMapFragments(physicalFirst),2); Test.equal(Engine.deriveMapFragments(supernaturalFirst),2); Test.deepEqual(Engine.deriveDestinationAvailability(physicalFirst),['final']); Test.deepEqual(Engine.deriveDestinationAvailability(supernaturalFirst),['final']); } finally { other.cleanup(); } });
  ...
  routeCase('201', 'cartões estáticos permanecem legíveis e não focáveis', function (current) {
    var legacy = current.root.querySelector('[data-destination-id="final"]');
    var before = Engine.snapshot(current.controller.getState());
    Test.includes(legacy.textContent, 'Bloqueado');
    Test.falsy(legacy.hasAttribute('tabindex'));
    Test.falsy(legacy.querySelector('input'));
    legacy.click();
    legacy.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
    legacy.dispatchEvent(new KeyboardEvent('keydown', { key: ' ', bubbles: true }));
    Test.deepEqual(Engine.snapshot(current.controller.getState()), before);
    Test.falsy(document.activeElement === legacy);
  });
  ...
  routeCase('203', 'zoom efetivo reflui rotas e heróis sem rolagem primária', function (current) { current.root.style.zoom='2'; var heroGrid=current.root.querySelector('.hero-grid'); var heroCards=heroGrid.querySelectorAll('.hero-card'); Test.equal(current.root.querySelectorAll('.route-card').length,3); Test.truthy(current.root.scrollWidth <= current.root.clientWidth); Test.equal(global.getComputedStyle(current.root.querySelector('.route-grid')).gridTemplateColumns.split(' ').length,1); Test.equal(global.getComputedStyle(heroGrid).gridTemplateColumns.split(' ').length,1); Array.prototype.forEach.call(heroCards,function(card){var bounds=card.getBoundingClientRect();var gridBounds=heroGrid.getBoundingClientRect();Test.truthy(bounds.left>=gridBounds.left&&bounds.right<=gridBounds.right);}); Test.truthy(current.root.querySelector('[data-action="depart"]')); }, { width: 320 });
  ...
  routeCase('209', 'seleção automática é anunciada por nome uma vez por transição', async function (current) { firstCompletion(current,'physical'); var automatic='Caminho selecionado automaticamente: Caminho das Vozes e dos Espelhos.'; Test.equal(Engine.derivePlayerView(current.controller.getState()).automaticSelectionAnnouncement,automatic); await new Promise(function(resolve){global.requestAnimationFrame(resolve);}); Test.includes(current.root.querySelector('[role="status"]').textContent,automatic); current.root.querySelector('[data-action="toggle-hero"][data-id="H1"]').click(); await new Promise(function(resolve){global.requestAnimationFrame(resolve);}); Test.falsy(current.root.querySelector('[role="status"]').textContent.includes(automatic)); });
  routeCase('210', 'texto longo conserva regiões em coluna estreita', function (current) {
    var cards = current.root.querySelectorAll('.route-card');
    Test.equal(cards.length, 3);
    Array.prototype.forEach.call(cards, function (card) {
      var rumor = card.querySelector('.route-rumor');
      var progress = card.querySelector('.route-progress');
      rumor.textContent += ' ' + rumor.textContent + ' ' + rumor.textContent;
      var cardBounds = card.getBoundingClientRect();
      var rumorBounds = rumor.getBoundingClientRect();
      var progressBounds = progress.getBoundingClientRect();
      Test.truthy(rumorBounds.left >= cardBounds.left && rumorBounds.right <= cardBounds.right);
      Test.truthy(progressBounds.left >= cardBounds.left && progressBounds.right <= cardBounds.right);
      Test.truthy(progressBounds.top >= rumorBounds.bottom);
      var radio = card.querySelector('.route-radio');
      if (radio) {
        Test.truthy(radio.getBoundingClientRect().right <= cardBounds.right);
      }
    });
    var routeGridBounds = current.root.querySelector('.route-grid').getBoundingClientRect();
    var heroGridBounds = current.root.querySelector('.hero-grid').getBoundingClientRect();
    var departBounds = current.root.querySelector('[data-action="depart"]').getBoundingClientRect();
    Test.truthy(heroGridBounds.top >= routeGridBounds.bottom);
    Test.truthy(departBounds.right <= current.root.getBoundingClientRect().right);
    Test.truthy(current.root.scrollWidth <= current.root.clientWidth);
  }, { width: 320 });
  ...
  routeCase('213', 'histórico longo mantém sequência crescente', function (current) {
    enterRouteAndRevealFirst(current, 'physical'); retreatToFormation(current);
    enterRouteAndRevealFirst(current, 'supernatural'); retreatToFormation(current);
    enterRouteAndRevealFirst(current, 'physical'); retreatToFormation(current);
    finishWithoutDeaths(current.controller);
    var history = global.expeditionQA.snapshot().actionHistory;
    Test.truthy(history.length > 50);
    Test.deepEqual(history.map(function (event) { return event.sequence; }), history.map(function (event, index) { return index + 1; }));
    Test.equal(new Set(history.map(function (event) { return event.sequence; })).size, history.length);
    var selectedDestinations = history.filter(function (event) { return event.type === 'destination_selected'; }).map(function (event) { return event.dungeon; });
    Test.includes(selectedDestinations, 'physical');
    Test.includes(selectedDestinations, 'supernatural');
  });
  ...
  routeCase('216', 'snapshot durante decisão conserva rota e posição', function (current) {
    enterRouteAndRevealFirst(current, 'physical', ['H1', 'H2', 'H3']);
    var approach = successfulApproach(current.controller.getState());
    Test.truthy(current.controller.dispatch({ type: 'CHOOSE_APPROACH', approachId: approach.id }).ok);
    var resultState = Engine.snapshot(current.controller.getState());
    var resultSnapshot = global.expeditionQA.snapshot();
    Test.equal(resultSnapshot.phase, 'approach_result');
    Test.equal(resultSnapshot.dungeon, 'physical');
    Test.equal(resultSnapshot.position, 1);
    Test.deepEqual(resultSnapshot.currentEncounter, resultState.currentEncounter);
    Test.deepEqual(resultSnapshot.assignments, resultState.assignments);
    Test.equal(current.controller.getState().pendingOutcome.approachId, approach.id);
    Test.truthy(current.controller.getState().pendingOutcome.success);
    Test.equal(current.controller.getState().phase, 'approach_result');

    var sacrifice = fixture();
    try {
      beginToFormation(sacrifice.controller, 216);
      enterRouteAndRevealFirst(sacrifice, 'physical', ['H4', 'H7', 'H8']);
      openFailure(sacrifice.controller);
      Test.truthy(sacrifice.controller.dispatch({ type: 'SELECT_VICTIM', heroId: 'H4' }).ok);
      var confirmationState = Engine.snapshot(sacrifice.controller.getState());
      var pendingBeforeSnapshot = JSON.parse(JSON.stringify(sacrifice.controller.getState().pendingOutcome));
      var historyBeforeSnapshot = JSON.parse(JSON.stringify(sacrifice.controller.getState().actionHistory));
      var confirmationSnapshot = global.expeditionQA.snapshot();
      Test.equal(confirmationSnapshot.phase, 'sacrifice_confirmation');
      Test.equal(confirmationSnapshot.dungeon, 'physical');
      Test.equal(confirmationSnapshot.position, 1);
      Test.deepEqual(confirmationSnapshot.currentEncounter, confirmationState.currentEncounter);
      Test.deepEqual(confirmationSnapshot.assignments, confirmationState.assignments);
      Test.deepEqual(confirmationSnapshot.actionHistory, confirmationState.actionHistory);
      Test.equal(sacrifice.controller.getState().pendingVictimId, 'H4');
      Test.deepEqual(sacrifice.controller.getState().pendingOutcome, pendingBeforeSnapshot);
      Test.deepEqual(sacrifice.controller.getState().actionHistory, historyBeforeSnapshot);
      Test.equal(sacrifice.controller.getState().phase, 'sacrifice_confirmation');
    } finally {
      sacrifice.cleanup();
    }
  });
  routeCase('217', 'seed fixa repete seleção e estado inicial', function (current) { clickRoute(current,'supernatural'); var a=global.expeditionQA.snapshot(); var other=fixture(); try { global.expeditionQA.setSeed(Number('217')); other.root.querySelector('[data-action="begin"]').click(); other.root.querySelector('[data-action="continue-intro"]').click(); clickRoute(other,'supernatural'); var b=global.expeditionQA.snapshot(); Test.deepEqual(a,b); } finally { other.cleanup(); } });
  ...
  routeCase('220', 'duas ordens são distinguíveis sem mudar candidatos finais', function (current) { var base=JSON.parse(JSON.stringify(current.controller.getState())); base.assignments.physical=['A1','A2','A3','A4','A5']; base.assignments.supernatural=['B1','B2','B3','B4','B5']; base.routeProgress={physical:5,supernatural:5,final:0}; base.selectedDungeonId='final'; var physicalFirst=JSON.parse(JSON.stringify(base)); var supernaturalFirst=JSON.parse(JSON.stringify(base)); function appendOrder(state,order){order.forEach(function(dungeonId){state.sequence+=1;state.actionHistory.push({sequence:state.sequence,type:'party_formed',dungeon:dungeonId,heroes:['H1','H2','H3']});});} appendOrder(physicalFirst,['physical','supernatural']); appendOrder(supernaturalFirst,['supernatural','physical']); var mountedA=fixture(1000,physicalFirst); var mountedB=fixture(1000,supernaturalFirst); try { Test.equal(mountedA.controller.getState().phase,'formation'); Test.equal(mountedB.controller.getState().phase,'formation'); Test.deepEqual(Engine.deriveFinalCandidates(mountedA.controller.getState()),Engine.deriveFinalCandidates(mountedB.controller.getState())); Test.deepEqual(mountedA.controller.getState().actionHistory.filter(function(event){return event.type==='party_formed';}).map(function(event){return event.dungeon;}),['physical','supernatural']); Test.deepEqual(mountedB.controller.getState().actionHistory.filter(function(event){return event.type==='party_formed';}).map(function(event){return event.dungeon;}),['supernatural','physical']); Test.equal(mountedA.controller.getState().selectedDungeonId,'final'); Test.equal(mountedB.controller.getState().selectedDungeonId,'final'); Test.includes(mountedA.root.textContent,Data.destinations.final.name); Test.includes(mountedB.root.textContent,Data.destinations.final.name); } finally { mountedA.cleanup(); mountedB.cleanup(); } });
})(window);
