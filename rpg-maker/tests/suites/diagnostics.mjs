...
const {validateBridgeAction,validateCapturedContext,validateCheckpoint}=require('../../The Dryland Drowned/js/plugins/Dryland_EventBridge.js');
...
canonicalCase('UT-046','public rule projections stay detached and never mutate campaign facts',()=>{
...
canonicalCase('UT-062','typed bridge arguments reject executable strings unknown enums before dispatch',()=>{
 ...
 assert.equal(reads,0);assert.deepEqual(validateBridgeAction({action:'TOGGLE_HERO',valueVariable:'22'},readVariable,144),{ok:true,field:'heroId',value:'H3'});assert.equal(reads,1);
