...
export async function execute(context){
 ...
 await player.until('formation');const before=await player.snapshot('tavern-before');
 ...
 await context.input.key('Tab');await player.until('formation');await context.shot('tavern-restored');
 ...
 for(const [index,name] of heroes.entries()){
  await player.choose(name,{mouse:index===0});await player.choose('Conversar');await player.ready();
  ...
  if(index===0){
   ...
   await consoleClick(context,player,'fastfwd');
   await context.wait(()=>$gameTemp.isExtendedFastForwardMode());
  }
  await player.until('formation');
 ...
 await consoleClick(context,player,'fastfwd');await context.wait(()=>$gameTemp.isExtendedFastForwardMode());
 await context.shot('fast-only-reread');await player.until('formation');
 ...
 await player.choose('Destinos');await player.until('destinations');await context.shot('available-and-locked-routes');await context.input.key('Escape');await player.until('formation');
 ...
 await player.click(point.x,point.y);await player.until('formation');await context.audio.stop();
 await captureNativeSave(context,'earned-tavern-prefix');
 if(process.env.DRYLAND_QA_FILES==='1'){
  for(const name of ['Gorvak','Elowen','Griznik']){await player.choose(name);await player.choose('Selecionar');}
  const full=(await player.snapshot('full-party')).campaign;await player.choose('Seraphina');await player.choose('Selecionar');await player.until('formation');assert.deepEqual((await player.snapshot('full-party-rejected')).campaign,full);
  await player.choose('Elowen');await player.choose('Retirar do grupo');await player.until('formation');assert.equal((await player.snapshot('removed-hero')).campaign.draftPartyIds.length,2);await player.choose('Elowen');await player.choose('Selecionar');await player.until('formation');
  ...
  await player.file(2);await player.until('formation');
