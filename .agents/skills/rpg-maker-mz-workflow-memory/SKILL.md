...
Keep a short current-state section: method/reference, pending criteria, valid evidence links, blockers and next action. Record exact paths, commands and inputs in the owning run/task; link immutable run details rather than copying chronology. Keep status truthful when work is partial or blocked.
