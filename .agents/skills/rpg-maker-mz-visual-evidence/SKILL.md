...
Reuse a reference whose build, fixture, assets/fonts, viewport/DPR, sensor and expected result remain equivalent; otherwise render it. Capture the implementation under matching conditions. Use one material image for a stable state and timestamped sequences for assigned temporal claims. Transient geometry drift invalidates capture even when final dimensions match. Store the bundle under the project QA evidence convention; otherwise use `docs/qa/evidence/<scenario>/<run>/`. Follow [the bundle template](assets/bundle.template.md).
