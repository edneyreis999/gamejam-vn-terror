...
description: Conduct reusable Loki web deep research with internet searches, source selection, credibility checks, citations, contradiction mapping, assumptions, gaps, and a structured handoff for `loki-deep-research` or a later approved workflow.
when_to_use:
...
- Contradictions are explicitly listed instead of collapsed into false
  certainty.
- Recommendations are proportional to evidence and name the next approved
  workflow when implementation, planning, durable documentation or policy
  change is needed.
