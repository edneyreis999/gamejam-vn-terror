...
Carregue `lf-web-deep-research` e, quando o dominio exigir, as
`<technology_required_skills>` disponíveis. Quando docs duradouros locais
precisarem contextualizar a web, leia somente as fontes locais mínimas
autorizadas. Dependencias condicionais nao se tornam obrigatorias para todo
profile.
...
- `loki-explorer`, read-only, para uma Pergunta Delimitada que estabeleca o
  baseline local; retorna fatos/evidencias compactos, nunca pesquisa externa,
  credibilidade, conflito multi-fonte ou schema do relatorio;
- `source-researcher`, read-only e paralelo, e padrao para cada trilha `deep` ou
  `deeper` e conserva fontes externas, multiplas familias, credibilidade e
  conflitos; em `quick/standard`, use-o para muitas fontes, contradicoes ou alto
...
3. Se depender do estado local, formule uma Pergunta Delimitada para
   `loki-explorer`; o workflow incorpora o baseline factual ao plano.
...
- Implementacao posterior recebe um handoff explícito para o workflow aprovado.
- `interview` para pergunta/escopo/destino ambiguo.
- `approval` para pesquisa longa/cara/paga/login/scraping/dados pessoais.
- Para recomendação que possa mudar o pacote, registre somente um candidato
  para futura avaliação autorizada; esta pesquisa não invoca Writer nem Auditor
  do pacote.
...
Encaminhe qualquer mudança do pacote como candidato para um workflow futuro
explicitamente autorizado; nao aplique. Qualquer mutacao de artefato
consolidado do pacote continua exigindo package root e envelope
`destination_scope: package`. Um relatorio transitorio preserva os targets do
command de pesquisa e nao concede mutacao.
