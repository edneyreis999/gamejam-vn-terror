...
**Autoria pelo editor:** conversas e cenas ficam em eventos nativos acessíveis a partir de eventos nomeados nos mapas. No evento Gorvak, escolher Conversar chama diretamente o Common Event da conversa pelo comando nativo Chamar evento comum. Aplicar o mesmo padrão às outras cenas; Common Events compartilhados podem conter a interação e seus ramos. A associação de conteúdo é o seletor da própria chamada nativa, sem cópia na configuração do Bridge nem comando próprio para despachar falas. Nomes públicos e demais dados de configuração usam campos acessíveis por Configuração do jogo; imagens e destinos das chamadas/transferências são selecionados nos comandos dos eventos. Textos, botões, estilos de foco/indisponibilidade, painéis, fundos, sacrifício, elenco e memorial ficam nos eventos. O Bridge fornece dados da campanha, recebe ações e a conclusão explícita de suas passagens, sem executar conversas. Complemento confirmado pelo usuário após a revisão da spec em 2026-09-12, registrado no [ADR-002](../planos/tasks/eventbridge-minimal-runtime/adrs/adr-002.md).
