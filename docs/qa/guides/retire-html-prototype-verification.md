...
Para verificações atuais, siga os comandos e pré-requisitos do [README MZ](../../../rpg-maker/README.md#verificar-alterações), selecionando os casos pelo contrato do incremento em análise.

Os comandos abaixo registram somente a execução histórica desta retirada. O validador foi removido pelo incremento EventBridge; este bloco não é executável na árvore atual:
...
A seleção histórica excluiu UT-059 e todos os ITs por usarem navegador. Ela não deve ser aplicada automaticamente a outro incremento: os corpos e imports dos casos podem mudar. O comando geral do README inclui integrações e tem pré-requisitos próprios. Use um checkout descartável quando precisar preservar as saídas anteriores, pois os testes escrevem resultados locais por ID.
