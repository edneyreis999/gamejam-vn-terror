...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    ...
    if (values.help) {
      console.log('Usage: node <runner>/directed-browser.mjs --project PATH --fixture PATH --case MODULE.mjs --adapter MODULE.mjs --output NEW_DIRECTORY\nInstall: npm ci --prefix <runner>, where <runner> is the directory containing this script and package-lock.json. Prepare Chrome separately. Browser width/height/DPR/locale/channel are explicit case.scenario.browser settings. Exit 0: collected (inspect report for pending review); exit 1: failure.');
    } else {
